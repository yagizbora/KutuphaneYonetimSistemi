        ...
        public async Task<IActionResult> ComplateRequest(ComplateRequest models)
        {
            try
            {
                using (var connection = _dbHelper.GetConnection())
                {
                    ...
                    string query = "UPDATE table_request_books SET is_complated = @is_complated,closed_subject_details = @closed_subject_details,request_start_time = @request_start_time,request_deadline = @request_deadline WHERE id = @id";
